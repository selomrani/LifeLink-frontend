...
axios.baseURL = 'http://localhost:8081/api';
const form = ref({
    firstName: '',
    lastName: '',
    email: '',
...
const handleRegister = () => {
    ...
    // Integration point for Laravel 12 API / Sanctum / Fortify
};
</script>
...
<template>
    <div class="min-h-screen bg-slate-50 text-slate-800 selection:bg-red-200 selection:text-red-900 font-inter">
        ...
        <header class="bg-white shadow-sm sticky top-0 z-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
                ...
                <div class="flex gap-4">
                    <router-link to="/login" class="px-5 py-2.5 text-sm font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-full transition-colors inline-flex items-center">
                        Log In
                    </router-link>
                </div>
            </div>
        ...
        <main class="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center p-4 py-12 relative overflow-hidden">
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-red-50 rounded-full blur-[100px] -z-10 opacity-60"></div>
            ...
            <div class="w-full max-w-lg">
                <div class="bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-2xl border border-slate-100 relative z-10">
                    ...
                    <div class="text-center mb-10">
                        ...
                        <p class="text-slate-500 text-balance">Become a donor today and start saving lives in your community.</p>
                    </div>
                    ...
                    <form @submit.prevent="handleRegister" class="space-y-5">
                        ...
                        <div class="space-y-2">
                            <label for="bloodType" class="text-sm font-bold text-slate-700 ml-1">Blood Type (Optional)</label>
                            <div class="relative">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div class="space-y-2">
                                <label for="password_confirmation" class="text-sm font-bold text-slate-700 ml-1">Confirm</label>
                                <input v-model="form.password_confirmation" type="password" id="password_confirmation" required minlength="8"
                                    class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:border-red-500 focus:ring-0 transition-all outline-none placeholder:text-slate-400"
                                    placeholder="••••••••">
                            </div>
                        </div>
                ...
                <p class="text-center mt-8 text-slate-600 font-medium mb-8">
                    ...
                    <router-link to="/login" class="text-red-600 hover:text-red-700 font-bold ml-1 transition-colors">Log in here</router-link>
                </p>
            </div>
        </main>
    </div>
